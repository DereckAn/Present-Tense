...
class DayLogViewModel: ObservableObject {
    ...
    // Función para obtener los días que tienen al menos una actividad
    // Devuelve un Set de DateComponents (solo año, mes, día) para usar con CalendarView
    func datesWithActivities(in month: DateInterval) -> Set<DateComponents> {
        let calendar = Calendar.current
        var dates = Set<DateComponents>()
        
        for activity in activities {
            // Asegurarse de que la actividad esté dentro del intervalo del mes visible
            if month.contains(activity.timestamp) {
                let components = calendar.dateComponents([.year, .month, .day], from: activity.timestamp)
    ...
    // Función para obtener las actividades de un día específico (ya la tenemos, pero la usaremos)
    func activities(for date: Date) -> [ActivityLog] {
        let calendar = Calendar.current
